...
class Main:
    ...
    def items_list(self):
        ...
        channel2 = [item for item in youtube.youtube(key=key).videos(self.youtube_link2) if 'ΝΤΟΚΙΜΑΝΤΕΡ' in item['title']]
        ...
        channel9 = youtube.youtube(key=key).playlist(self.youtube_link9); ch9 = channel9

        # dictionary comprehension not compatible with python 2.6:
        # channel1_fixed = [{key: re.sub('^\d{2} ', '', item[key]) if key == 'title' else value for key, value in item.items()} for item in channel1]
        # channel2_fixed = [{key: item[key].partition(' - ')[2] if key == 'title' else value for key, value in item.items()} for item in channel2_fixed]

        ch1 = [dict((k, re.sub('^\d{2} ', '', item[k]) if (k == 'title') else v) for k, v in item.items()) for item in channel1]
        ch2 = [dict((k, item[k].partition(' - ')[2] if (k == 'title') else v) for k, v in item.items()) for item in channel2]
        ch3 = [item for item in channel3 if int(item['duration']) >= 600]
        ch4 = [item for item in channel4 if int(item['duration']) >= 900]
        ch8 = [dict((k, item[k].partition(' (ΝΤΟΚΙΜΑΝΤΕΡ')[0] if (k == 'title') else v) for k, v in item.items()) for item in channel8]

        self.list = ch1 + ch2 + ch3 + ch4 + ch5 + ch6 + ch7 + ch8 + ch9

        self.data = [{'title': 'Η ταχύτητα του φωτός', 'image': u'https://i.ytimg.com/vi/oLjGJOP9Cb4/mqdefault.jpg', 'url': self.youtube_extra, 'duration': 2629}]
